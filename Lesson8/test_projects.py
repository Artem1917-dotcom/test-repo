import pytest
from Lesson8.projects_client import ProjectsClient
...
class TestProjectsAPI:
    ...
    def test_update_project_positive(self, created_project_id):
        response = self.client.update_project(
            created_project_id, self.update_data)
        assert response.status_code == 200
    ...
    def test_update_project_negative_not_found(self):
        response = self.client.update_project(
            "non-existent-project-id-12345", self.update_data)
        assert response.status_code == 404
